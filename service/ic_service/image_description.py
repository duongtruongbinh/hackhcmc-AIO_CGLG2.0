...
def image_summarizing_gpt(img_str, options, CLIP_class, YOLOw_df, gpt):
    results = {}
    if len(options) == 0:
        choices = default_opts[CLIP_class]
    else:
    ...
    for choice in choices:
        ...
        # Create the prompt for chat model
        ...
        description = response.choices[0].message.content.strip()  # Use message attribute
        time.sleep(1)
        results[f'{choice}'] = description
...
def extract_hashtag_gpt(gpt, description):
    content =  f"Create a list of the top ten best-suited hashtags for the scene from the following image description: {description}. \
                                        Consider relevant themes, locations, and activities in the scene. \
                                        For example: Hashtags: [#bar, #pub, #restaurant, #conveniencestore, #supermarket, \
                                        #event, #party, #celebration, #gathering, #happyhour, #funtime.]"
    ...
    for line in content.split('\n'):
        if '. ' in line:
            hashtags_list.append(line.split('. ')[1].strip())
        else:
