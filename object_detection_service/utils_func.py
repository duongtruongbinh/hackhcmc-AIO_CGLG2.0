...
def process_ocr_data(df_OCR, image):
    ...
    df_OCR = df_OCR[['xmin', 'ymin', 'xmax', 'ymax',
                     'confidence', 'name', 'brand_class']]
...
def owlv2_detection(im):
    ...
    print(target_sizes)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    height_ratio, width_ratio = 1, 1
    width = target_sizes[0][0]
    height = target_sizes[0][1]
    if width > height:
...
def process_image(image_path, df_OCR):
    ...
    for i, box1 in df_owlv2.iterrows():
        for j, box2 in df_owlv2.iterrows():
            # Use threshold=1.0 for 100% coverage
    ...
    for _, obj in df_OD.iterrows():
        ...
        new_name = obj['name'] + ' ' + \
            ' '.join(matched_brands) if matched_brands else None
    ...
    filtered_df = filtered_df[['xmin', 'ymin', 'xmax',
                               'ymax', 'confidence', 'name', 'brand_class']]
    ...
    heineken_brand_count_df = sub_brand_count_df[~sub_brand_count_df['brand_class'].str.contains(
        'competitor')]
    heineken_brand_count_df.rename(
        columns={'brand_class': 'heineken_object'}, inplace=True)

    competitor_brand_count_df = sub_brand_count_df[sub_brand_count_df['brand_class'].str.contains(
        'competitor')]
    ...
    #         cv2.rectangle(annotated_image, (int(row['xmin']), int(
    #             row['ymin'])), (int(row['xmax']), int(row['ymax'])), (0, 0, 255), 2)
    #         cv2.putText(annotated_image, row['name'], (int(row['xmin']), int(
    #             row['ymin']) - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

    # for _, row in final_df.iterrows():
    #     if row['brand_class'] is not None:
    #         cv2.rectangle(annotated_image, (int(row['xmin']), int(
    #             row['ymin'])), (int(row['xmax']), int(row['ymax'])), (255, 0, 0), 2)
    #         # cv2.putText(annotated_image, row['brand_class'], (int(row['xmin']), int(row['ymin']) - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    #         cv2.putText(annotated_image, str(row['brand_class']), (int(row['xmin']), int(
    #             row['ymin']) - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
